  ...
  return (
    <div className="mx-auto max-w-2xl space-y-4 p-4 md:p-6">
      ...
      <Card className="overflow-hidden">
        ...
        <div className="space-y-3 p-4 sm:p-5"><h3 className="font-bold">Chi tiết nhận / đóng tiền</h3>{receipt.lines.map((line, i) => <div key={line.periodId} className="rounded-lg border p-3.5"><div className="flex items-start justify-between gap-3 border-b pb-2.5"><div className="min-w-0"><p className="font-semibold">{i + 1}. {line.groupName}</p>{line.groupCode && <p className="mt-0.5 text-xs text-muted-foreground">{line.groupCode}</p>}</div><p className="shrink-0 text-sm font-semibold">Kỳ {line.periodNumber}/{line.totalPeriods}</p></div><div className="mt-3 grid gap-x-5 gap-y-2 text-sm sm:grid-cols-2"><InfoRow label="Giá thăm" value={formatVND(line.bidAmount)} /><InfoRow label="Chân sống / chết" value={`${line.liveShares} / ${line.deadShares}`} /><InfoRow label="Tiền đóng" value={formatVND(line.payAmount)} /><InfoRow label="Thực chi người hốt" value={formatVND(line.receiveAmount)} />{line.feeAmount > 0 && <InfoRow label="Tiền thảo (đã trừ tiền hốt)" value={formatVND(line.feeAmount)} />}{line.profitAmount > 0 && <InfoRow label="Lợi hụi" value={formatVND(line.profitAmount)} />}</div></div>)}</div>
        <div className="border-t bg-muted/20 p-4 sm:p-5"><div className="space-y-2 text-sm"><TotalRow label="Lợi hụi chân sống" value={receipt.totalProfit} /><TotalRow label="Tổng tiền đóng hụi" value={receipt.totalPay} /><TotalRow label="Tổng thực chi người hốt" value={receipt.totalReceive} /><TotalRow label="Tiền thảo chủ hụi giữ" value={receipt.totalFee} /><TotalRow label="Tất toán / điều chỉnh" value={receipt.settlementAmount} signed /><TotalRow label="Đã thu / chi" value={receipt.paidAmount} /></div><div className="mt-4 border-t pt-4 text-center"><p className="text-sm font-bold uppercase">{receipt.status === "cancelled" ? "Phiếu đã hủy" : receipt.remainingAmount <= 0 ? "Đã thanh toán đủ" : receipt.direction === "collect" ? "Hụi viên còn phải đóng cho chủ hụi" : receipt.direction === "pay" ? "Chủ hụi còn phải giao cho hụi viên" : "Phiếu đã cân bằng"}</p><p className="mt-1 text-2xl font-black">{formatVND(receipt.remainingAmount)}</p></div></div>
