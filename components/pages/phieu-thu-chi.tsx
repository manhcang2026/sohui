...
type ReceiptLine = { periodId: string; groupId: string; groupCode: string; groupName: string; periodNumber: number; totalPeriods: number; bidAmount: number; liveShares: number; deadShares: number; payAmount: number; huiAmount: number; receiveAmount: number; feeAmount: number; profitAmount: number }
type Receipt = { member: MemberRow; lines: ReceiptLine[]; groupCount: number; totalShares: number; liveShares: number; deadShares: number; totalPay: number; totalHuiAmount: number; totalReceive: number; totalFee: number; totalProfit: number; db: ReceiptDbRow | null; payments: PaymentRow[]; settlementAmount: number; netAmount: number; paidAmount: number; remainingAmount: number; direction: "collect" | "pay" | "balanced"; status: "open" | "partial" | "paid" | "cancelled" }
...
export function PhieuThuChiPage() {
  ...
  const receipts = useMemo(() => {
    ...
    for (const period of periods.filter((x) => isCompleted(x.status) && x.scheduled_date === selectedDate)) {
      ...
      // TỔNG HỐT HỤI = tổng tiền đóng của 23 chân còn lại.
      const huiAmount = Math.max(0, pot)
      // CHỦ HỤI PHẢI GIAO = tổng hốt hụi - tiền thảo.
    ...
    for (const [memberId, lines] of linesByMember) {
      ...
      const totalPay = lines.reduce((n, x) => n + x.payAmount, 0); const totalHuiAmount = lines.reduce((n, x) => n + x.huiAmount, 0); const totalReceive = lines.reduce((n, x) => n + x.receiveAmount, 0); const totalFee = lines.reduce((n, x) => n + x.feeAmount, 0); const totalProfit = lines.reduce((n, x) => n + x.profitAmount, 0)
  ...
  return (
    <div className="mx-auto max-w-2xl space-y-4 p-4 md:p-6">
      ...
      <Card className="overflow-hidden">
        ...
        <div className="space-y-3 p-4 sm:p-5"><h3 className="font-bold">Chi tiết nhận / đóng tiền</h3>{receipt.lines.map((line, i) => <div key={line.periodId} className="rounded-lg border p-3.5"><div className="flex items-start justify-between gap-3 border-b pb-2.5"><div className="min-w-0"><p className="font-semibold">{i + 1}. {line.groupName}</p>{line.groupCode && <p className="mt-0.5 text-xs text-muted-foreground">{line.groupCode}</p>}</div><p className="shrink-0 text-sm font-semibold">Kỳ {line.periodNumber}/{line.totalPeriods}</p></div><div className="mt-3 grid gap-x-5 gap-y-2 text-sm sm:grid-cols-2"><InfoRow label="Giá thăm" value={formatVND(line.bidAmount)} /><InfoRow label="Chân sống / chết" value={`${line.liveShares} / ${line.deadShares}`} /><InfoRow label="Tiền đóng" value={formatVND(line.payAmount)} /><InfoRow label="Tổng hốt hụi" value={formatVND(line.huiAmount)} />{line.feeAmount > 0 && <InfoRow label="Trừ tiền thảo" value={formatVND(line.feeAmount)} />}{line.profitAmount > 0 && <InfoRow label="Lợi hụi" value={formatVND(line.profitAmount)} />}</div></div>)}</div>
        <div className="border-t bg-muted/20 p-4 sm:p-5"><div className="space-y-2 text-sm"><TotalRow label="Lợi hụi chân sống" value={receipt.totalProfit} /><TotalRow label="Tổng tiền đóng hụi" value={receipt.totalPay} /><TotalRow label="Tổng hốt hụi" value={receipt.totalHuiAmount} /><TotalRow label="Trừ tiền thảo" value={receipt.totalFee} /><TotalRow label="Tất toán / điều chỉnh" value={receipt.settlementAmount} signed /><TotalRow label="Đã thu / chi" value={receipt.paidAmount} /></div><div className="mt-4 border-t pt-4 text-center"><p className="text-sm font-bold uppercase">{receipt.status === "cancelled" ? "Phiếu đã hủy" : receipt.remainingAmount <= 0 ? "Đã thanh toán đủ" : receipt.direction === "collect" ? "Hụi viên còn phải đóng cho chủ hụi" : receipt.direction === "pay" ? "Chủ hụi còn phải giao cho hụi viên" : "Phiếu đã cân bằng"}</p><p className="mt-1 text-2xl font-black">{formatVND(receipt.remainingAmount)}</p></div></div>
