...
import { authPasswordFromPin } from "@/lib/auth/passcode"
...
export async function GET(request: NextRequest) {
  try {
    ...
    const { data, error } = await auth.supabase
      .from("app_users")
      .select(
        "auth_user_id, email, display_name, role, member_id, is_active, created_at, updated_at",
      )
      .order("display_name")
...
export async function PATCH(request: NextRequest) {
  try {
    ...
    if (action === "reset_pin") {
      const { error } = await auth.supabase.auth.admin.updateUserById(
        userId,
        { password: authPasswordFromPin("0000") },
      )
      if (error) throw error
